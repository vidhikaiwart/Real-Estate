...
const WhatsAppCampaign = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      console.log('Sending WhatsApp campaign...')
      console.log('Selected lead IDs:', selectedLeads)
      console.log('Message:', formData.message)
      
      const data = await campaignsAPI.sendWhatsApp(
        formData.message,
        selectedLeads
      )

      console.log('WhatsApp campaign response:', data)
      setSuccess(`WhatsApp campaign sent successfully to ${data.sent || data.total || selectedLeads.length} leads!`)
      setTimeout(() => {
        navigate('/campaigns')
      }, 2000)
    } catch (err) {
      console.error('WhatsApp campaign error:', err)
      setError(err.message || 'Failed to send WhatsApp campaign')
    } finally {
      setLoading(false)
    }
  }
    ...
    <div>
      <div className="mb-8">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-semibold text-gray-900">WhatsApp Campaign</h1>
            <p className="text-gray-600 mt-1">Send bulk WhatsApp messages to selected leads</p>
          </div>
        </div>
      </div>
